...
| 문서 | 500줄 | ⚠️ **이 행만 유래가 기록되지 않았다** — 아래 주 |
| 괄호 중첩 | 금지 | 한 문장에 한 생각 |

⚠️ **500줄의 유래는 남아 있지 않다.** 이 표의 3열은 「이유」인데 다른 넷은 실제로 이유이고
(*"가로 스크롤 없이 읽힌다"*·*"표는 대조용이다"* 등) **이 행만 처방**(*"넘으면 주제별로 나눈다"*)이
들어가 있었다. 도입 커밋은 *"상한의 **근거**"* 를 넣었다고 적었으므로 **빠뜨린 줄 모르고 있었다.**

⚠️ **칸이 비어 있지 않아서 안 걸렸다.** 빈 칸이면 누가 채웠을 텐데, 처방이 들어가 있으면
문장이 자연스럽게 읽혀 **검산을 통과한다.** 「없음」과 「종류가 다름」은 다른 축이고 후자가 더
오래 남는다 — [`philosophy.md`](../philosophy.md) §계측 단위의 *"틀린 값보다 단위가 어긋난 정답이
위험하다"* 와 같은 형태이며, 여기서 어긋난 단위는 값이 아니라 **칸의 종류**다.

⇒ **이 값을 바꾸려면 근거부터 세운다.** 상한에 걸려서 올리는 것은 근거가 아니다 — 그러면
다음에 또 올리게 되고, 그때도 왜 그 값인지 아무도 모른다. **현행 유지는 「검토하고 유지」가
아니라 「유래 미확인」** 이다(빠뜨린 것과 구분하려고 적는다).

**넘었을 때의 처방은 분할이고, 형태는 「허브 + 하위 정본」이다** — 허브에 요약과 포인터를 남기고
상세를 하위 문서로 내린다. 선례: [`git.md`](git.md) → [`git/worktree.md`](git/worktree.md) ·
[`k8s.md`](k8s.md) → [`k8s/cnpg.md`](k8s/cnpg.md)·[`k8s/spark-connect.md`](k8s/spark-connect.md).
⚠️ **분할 비용은 앵커가 아니라 산문 속 §번호 참조다** — `#앵커` 링크는 `--links`가 잡지만
*"k8s.md §9-3"* 처럼 **본문에 적힌 절 번호는 어떤 게이트도 잡지 않는다.** 절을 옮기면
그 참조들이 조용히 낡는다. 분할 전에 `§` 참조를 먼저 센다.
...
uv run scripts/doc_lint.py --links         # 링크·앵커 + 볼트 참조 + 위키 인덱스 (전역)
```
...
⚠️ **`--links`는 축이 셋이고 단위가 제각각이다.** "3종을 본다"까지만 적으면
**값은 맞는데 라벨이 세는 대상이 어긋난** 상태를 다시 만든다.

| 축 | 방향 | 계측 단위 | 0건의 뜻 |
| --- | --- | --- | --- |
| 링크·앵커 | 문서 → 대상 | **깨진 참조 수** | 통과 |
| 볼트 참조 | `docs/` → 볼트 | **깨진 참조 수** | 통과. 단 `$OBSIDIAN_VAULT`가 없으면 **「검사 안 함」**으로 따로 찍힌다 |
| 위키 인덱스 | 파일 → 인덱스(**반대 방향**) | **노트 수** | 아래 단서를 함께 읽는다 |

- **`unindexed`의 단위는 노트 수다** — 한 노트가 `Home.md`·`_Sidebar.md` **양쪽에 다
  빠져도 1건**이지 2건이 아니다. 인덱스별로 세면 같은 누락이 두 번 잡힌다.
- **인덱스 파일 자체가 없으면 `missing-index`가 나고 노트별 판정은 생략된다.**
  ⇒ 그때의 `unindexed 0건`은 **통과가 아니다.** 부정 결과를 읽기 전에
  `missing-index`가 함께 났는지부터 본다.
- **모집단은 `wiki/*.md` 평면 글롭**에서 인덱스 2종을 뺀 것이다. `rglob`이 아닌 이유는
  [`publishing.md`](publishing.md) §4-1이 위키 하위 디렉터리를 **금지**하기 때문이다 —
  검사기가 규약이 금지한 배치를 먼저 인정하면 안 된다.
