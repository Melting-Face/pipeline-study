...
⚠️ **매칭 술어까지 공용 모듈로 합쳤고** 두 가드가 같은 술어를 부른다. 다만 **축마다 방향이
반대라** "일관성"으로 통일하면 **반드시 한쪽이 fail-open**이 된다 — 막는 축(`deny`·`except`)은
넓게·대소문자 무시로, 여는 축(`allow`)은 좁게·대소문자 구분으로 걸리는 편이 안전하다.
**부르는 쪽이 축을 바꿔 부르면 갈리므로** 대조군 셀은 여전히 필요하다(축이 좁아졌을 뿐이다).

| 축 | 방향 | 평가 순서 | 매칭 |
| --- | --- | --- | --- |
| `except` | 막는다 | **가장 먼저** | `/`면 접두어, 아니면 완전일치 · **대소문자 무시** |
| `deny` | 막는다 | 다음 | 항상 접두어(넓게) · **대소문자 무시** |
| `allow` | 허용한다 | 마지막 | `/`면 하위 전체, 아니면 완전일치 · **대소문자 구분** |
