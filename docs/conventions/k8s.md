...
- **오퍼레이터 watch는 장시간 후 죽는다 — 상태 필드만 믿지 않는다**(실측).
  ...
  **탈출 실측은 기대 창을 근소하게 넘을 수 있다** — 폴링 경계에 자산 종료 처리 지연이 더해진다.
  ⚠️ 그 초과분이 *설명된다*는 것과 *검증됐다*는 것은 다른 축이고 표본이 1건이라 **재현은 미확인**이다.
  기대 창을 상한이 아니라 **근사**로 읽어야 "창 밖이니 이 분기가 아니다"로 오판하지 않는다.
  실측값은 `$OBSIDIAN_VAULT/status/observations.md`에 있다.
...
- **크리덴셜은 SQL DDL에 쓰지 않는다** — `sql-client`가 실행문을 **그대로 echo**해 터미널·로그에 평문이 남는다
  ...
  4. **무유출 4점 확인이 조건이다** — ⓐ 컨테이너 로그 ⓑ 파드 파일시스템 ⓒ **JobManager REST 로그 API**
     ⓓ **Dagster run 로그**. 넷 다 확인해야 통과이고, 하나라도 미확인이면 **미확인**이지 통과가 아니다.

     ⚠️ **ⓓ는 자동화가 새로 만든 축이다.** 조건 2·4는 *사람이 터미널에서 `kubectl exec`를 친다*는
     전제로 쓰였다. 그때 stdout은 터미널에 흩어지고 끝난다. 그런데 Dagster 자산이 대신 실행하면
     같은 stdout이 **run 로그(메타 Postgres + S3 compute log)** 에 착지하고, 그 UI는
     `dagster.localtest.me` Ingress로 **인증 없이** 열려 있다. 조건 2가 금지한 "컨테이너 로그"보다
     **오래 남고 더 넓게 읽힌다**.
     ⇒ **실행 주체가 사람에서 자동화로 바뀌면 조건을 다시 센다.** 같은 명령이라도 출력이
     어디에 착지하는지가 달라진다(원칙 7 — 통제는 명령이 아니라 **경로**에 걸린다).
     ⇒ 현행 구현(`defs/flink/resources.py`)은 이 축을 **파드 안 마스킹**으로 닫는다:
     `grep -F`(리터럴 — `sed`는 비밀번호에 정규식 메타문자가 섞이면 **조용히** 실패한다)로
     비밀값을 품은 줄을 지우고, **센티널 프로브**로 그 마스킹이 매 실행 실제로 작동하는지 확인한 뒤
     잡을 돌린다. 마스킹 실패·비밀값 미설정은 **잡을 실행하기 전에** 종료 코드로 죽는다(fail-closed).
