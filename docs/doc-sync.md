...
| **열린 작업**(공개 가능) | **GitHub Issues** — 규약은 [`conventions/issue.md`](conventions/issue.md) | 해당 규칙 문서의 **「담지 않는 것」 선언**<br/>라벨은 `area:*` 1개와 `prio:*` 1개를 **둘 다** 붙인다.<br/>등록 전에 §실무 규칙 7 **축 3(위협)** 으로 공개 가능 판정을 먼저 한다. |
...
7. 🔴 **`docs/`에 무엇을 남길지는 3축으로 판정한다.** 순서가 있다 — 앞의 둘이
   ...
   **보증 범위 — 기계가 보는 것은 축 2의 하위 규칙 하나뿐이다.** `doc_lint.py`의 **기본 검사**가
   날짜 표기를 잡고 커밋을 막는다(`doc-lint` 훅). 그러나 그것은 *"이 자리에 날짜가 있다"* 까지다 —
   **날짜 없이 쓴 실측 수치**(*"피크 84%"*·*"22888 MiB"*)는 관측 범위 **밖**이고, 문장이 재현
   가능한지(축 1)와 공개해도 되는지(축 3)는 **보지 않는다**. 나머지는 사람·에이전트가 문장을 쓸 때
   **스스로 묻는 테스트**이며 자동 게이트가 없다.
   ⇒ **`--dates` 0건을 "낡는 문장이 없다"로 읽지 않는다.**
