...
- ⚠️ **잔여물 — 해소(2026-08-23)**: 손상된 `smoke`/`smoke_seed` 테이블은 카탈로그에서 지웠으나
  **객체 53개가 orphan으로 남아 있었다**. `scripts/spark_connect_smoke.py`에 `DROP TABLE ... PURGE`를
  붙이고 **사람이 고아를 일괄 삭제**해 `s3://warehouse/smoke/`는 **0개**가 됐다
  (prefix 자체가 사라짐 — 2026-08-23 `warehouse` 버킷 전수 관측).
  🔴 **유지보수 잡이 지운 것이 아니다.** *"현행 유지보수 잡으로는 정리되지 않는다"* 는 서술은
  **여전히 참**이고, 없어진 것은 **대상**이지 **커버리지 공백**이 아니다
  ([../test.md](../test.md) §`iceberg_maintenance_job` 커버리지 공백 ⓑ).
  🔴 같은 계열의 **현행 실례**는 남아 있다 — `s3://warehouse/chk/` **6개(15.0 KiB)**.
  네임스페이스 `chk`는 카탈로그에 **있는데 테이블이 0개**라 인자로 지목할 대상이 없다.
  **출처는 `미확인`**(어느 작업이 만들었는지 관측하지 않았다).
