...
> 깨지므로**, 이번에는 **기본안 (나) 시연 창 한정 + 즉시 회수**로 진행했다. 상주분 실측은 나왔고
> (아래 §자원 실측) **규약 개정 여부는 사용자 결정 대기**다 — 이 문서는 규약을 바꾸지 않는다.
...
- 🔴 **규약 개정 여부는 사용자 결정 대기**다. 이번에는 **기본안 (나) 시연 창 한정 + 즉시 회수**로
  진행했고 **회수 후 기준선으로 정확히 복귀**했다. 정본 [conventions/k8s.md](../conventions/k8s.md) §9-3은
  **이 문서가 바꾸지 않는다.**
...
- **자원·동시 기동**: JM+TM는 **배치(Spark)와 동시 실행이 허용**된다(2026-08-22 실측 피크
  ...
  **Redpanda 미도입**(도입 시 경계 재계산). 정본 [conventions/k8s.md](../conventions/k8s.md) §9-3,
  ...
  2026-08-23에 **처음 측정**됐고 **규약 개정 여부는 결정 대기**다(위 §자원 실측).
...
### 🔴 닫지 못한 것 — 경계 ①의 전제 (측정 완료 · **사용자 결정 대기**)
...
| 선택지 | 내용 | 비용 |
| --- | --- | --- |
| (가) | 경계 ① 자체를 개정한다(TM 상주를 예산에 반영) | 예산 재계산 + 규약 정본 개정 |
| (나) | **시연 창 안에서만 돌리고 그 자리에서 회수한다** | 규약 무개정. 회수 규율에 그대로 얹힌다 |

- **기본안은 (나)** 다 — 경계 ①을 건드리지 않고, 이미 있는 회수 규율(검증이 끝나는 그 자리에서 내린다)에
  스트리밍 잡을 얹는다. **2026-08-23 실증도 (나)로 진행**했고 회수 후 기준선으로 정확히 복귀했다.
- 🔴 **바뀐 것은 하나 — 이제 수치가 있다.** 종전 이 절의 공백(`미측정`)이 채워져
  스트리밍 상주 피크가 **`4750m (59%)` / `8772Mi (39%)`** 로 관측됐다(위 §자원 실측).
  **배치 동시 피크(84% / 52%)보다 낮다.**
- 🔴 **그래도 이 절은 규약 정본을 바꾸지 않는다.** **"예산이 통과한다"와 "경계 ①의 전제가 성립한다"는
  다른 축**이고, §9-3 개정 여부는 **사용자 결정 대기**다. 결정 전까지 스트리밍 상시 운전은 하지 않는다.
  자원 축의 서술은 [../resource-sizing.md](../resource-sizing.md) §(C)·§(C-2).
