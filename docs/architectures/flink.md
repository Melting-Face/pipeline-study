...
**Spark ↔ Flink Iceberg 왕복**을 배치·스트리밍 양쪽에 걸쳐 **설계했다.**

**저장소에 있는 것**(= 커밋된 선언):
세션 클러스터 매니페스트, 잡별 SQL ConfigMap, 배치 경로를 관통하는 Dagster 자산
(`defs/flink/`), 그리고 체크포인트·RocksDB·S3 상태 저장 설정.

**클러스터에서 관측된 것**: 정본은 [conventions/k8s.md](../conventions/k8s.md) §9-2
「검증 상태」다 — **여기에 그 범위를 복제하지 않는다.** 복제하면 두 곳이 각자 낡고,
이 문서가 한 번 그렇게 낡아 자기 안에서 등급이 갈렸다.

⚠️ **「배포돼 있다」는 계측 단위가 둘이다** — *저장소의 매니페스트*인가
*클러스터의 리소스*인가. 위 두 문단이 그 둘을 가른다.
자기 등급 서술은 아래 §Dagster 배선이 갖는다.
...
### Dagster 배선 — 회수 규율을 사람이 아니라 코드가 진다

**배치 경로에 한해** Dagster 자산(`defs/flink/`)이 세션 클러스터의 수명을 쥔다:
`FlinkDeployment` 기동 → JM 준비 대기 → `sql-client` 실행 → **회수**를 자산 하나가 관통하고,
회수는 `finally`에 있어 **실패해도 내려간다**.

🔴 **이 배선의 존재 이유는 편의가 아니라 위 회수 규율이다.** 규약은 문서에 있었는데
실제로 **반나절 넘게 샜고**, 발견 경로는 성능 이상이 아니라 "안 쓰는 것 정리"였다.
**회수를 트리거하는 주체가 없으면 규약은 조용히 샌다** — 그 주체를 코드로 만든 것이다.

⚠️ **순서가 규칙이다** — 회수가 JM을 지우면 진단 근거도 함께 사라지므로
`teardown()` **전에** JM 로그를 회수한다.

**제출 형태의 선택지는 둘 다 닫혀 있었다**(그래서 `exec` 스트림이다):

| 후보 | 왜 못 쓰나 |
| --- | --- |
| `FlinkSessionJob` CRD | 오퍼레이터가 jar를 JM에 업로드하는 구조라 `jarURI: local://`이 죽는다. 이 저장소의 배치 잡은 jar가 아니라 **SQL ConfigMap + `sql-client.sh`** 다 |
| `PipesK8sClient` | K8s Job/Pod를 띄우는 방식이라 [conventions/k8s.md](../conventions/k8s.md) §C5 조건 2("stdout이 컨테이너 로그가 되는 형태 금지")와 **정면 충돌**한다 |

⚠️ **자동화가 노출 축을 하나 만든다** — 사람이 치던 `exec`의 stdout이 이제 **Dagster run 로그**에
착지한다(§C5 조건 4의 ⓓ). 파드 안 마스킹(`grep -F` + 센티널 프로브)으로 닫았다.

⚠️ **현재 등급은 「선언」이다.** 코드는 main에 있으나 `terraform apply`·이미지 재빌드 전이라
클러스터는 아직 움직이지 않았다. **스트림 잡 수명주기 관리는 미착수**이고,
배치에서 성립한 것을 그대로 일반화하지 않는다(바로 아래 절).
