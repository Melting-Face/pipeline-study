...
- **주입 단위는 스킬 디렉터리가 아니라 `SKILL.md` 한 파일이다**(실측 — `dagster-integrations`를
  프리로드한 프로브가 도구 0회로 `SKILL.md` 본문은 원문 인용했고 `references/storage.md`는 `NOT-IN-CONTEXT`로 답했다).
  ⇒ **`Read`가 없는 워커에게 `references/` 경로를 적으면 죽은 참조다.**
  프리로드 단서에는 주입 대상과 안내 대상을 갈라 적는다.
- **"워커에 `Skill` 도구가 없다"는 서술을 폐기한다**(실측으로 반증).
  ...
  - **구 근거는 동어반복이었다** — 잰 것은
    *"`tools:`를 열거했고 그 목록에 `Skill`이 없는 워커 2종의 자기보고"* 뿐인데
    `tools:`가 화이트리스트이므로 **정의상 참**이다
    ([`../philosophy.md`](../philosophy.md) §계측 단위 ①갈래 + ⑥부재의 확언).
    ⚠️ 여기 있던 `grep -L 'Skill' .claude/agents/*.md` **검산은 걷어냈다** — 계수가 낡았을 뿐
    아니라 **세는 단위가 어긋나 있었다.** 그 명령은 파일 어디에든 `Skill` 문자열이 있으면 세는데,
    이 문장이 말하는 것은 **프론트매터 `tools:` 선언**이다. 값이 맞아도 라벨이 다른 그 형태다.
  - ⚠️ **아래 「그럼에도 열지 않는다」는 `Skill` 미부여 시절의 서술이다.** 그 뒤 제한 수단
    (`scripts/skill_gate_guard.py`)을 먼저 만들고 **일부 워커에 `Skill`을 열었다** —
    현행 정본은 [`../../CLAUDE.md`](../../CLAUDE.md) §AI 세션 운영이다. **이 절은 교정 대기**이며,
    교정 없이 인용하면 현행과 반대되는 결론이 나온다.
  ...
  - ⚠️ **현행 워커의 `Skill` 부재는 정적 실측 + 연역**이고, 강제 호출로 얻은 런타임 에러는 **없다**
    ...
    ⚠️ **계수를 여기 박지 않는다 — 분모가 세션 중에도 움직인 값이다**(`director.md` 삭제로
    한 차례 줄었고, 그 뒤로도 줄었다). 박아 두면 **틀린 값이 아니라 안 자라는 값**이 되어
    검산을 통과하며 남는다. 세려면 그 시점에 센다:

    ```shell
    ls -1 .claude/agents/*.md | wc -l        # 분모 — .md 파일 총수(전문 워커 수가 아니다)
    grep -l 'Skill' .claude/agents/*.md | wc -l
    ```

    ⚠️ 위 분모가 세는 것은 **`.claude/agents/`의 `.md` 파일 총수**이지 전문 워커 수가 아니다.

## 프리로드 자격

- **프리로드 조건을 "lock 등재분" → "lock 등재 ∧ `security` 검토 완료분"으로 강화한다**.
...
- **세 번째 축 「상시성」을 신설한다**. 앞의 두 축은 **둘 다 안전 축**이라 **비용 축이 0개**였고,
  ...
  **안전 축이 비용 축과 거래된다.** 루브릭의 **"게이트 축과 채점 축을 섞지 않는다"** 와 같은 형태의 함정이다.

  - **근거**: 프리로드는 배정 1건당 **`≈ 4.95 × T` 환산 입력토큰**을 **무조건** 진다.
    배정 1건이 무는 **서브에이전트 왕복 수**는 세션 로그로 실측하고(값과 표본 크기는 볼트),
    캐시 계수 4.95는 표준 단가 **가정**이다. `Read` 대비 이득은 발동률 `p`에
    비례하므로 **`p ≲ 0.5`면 순손실**이다 —
    `p=1.0`은 거의 동률(+0.20 T)이나 `p=0.5`는 **2.6배**, `p=0.2`는 **4배** 손해.
...
- 🔴 **오타난 스킬명은 워커에게 아무 신호도 주지 않고, 경고는 로그에만 남는다**(실측 — 종전 `미검증` 해소).
  ...
  찍혔다(대조군: 정상 프로브 로그에 `[Agent: probe-b2] Preloaded skill 'dagster-integrations'`
  — **관측 경로 생존 확인**).
