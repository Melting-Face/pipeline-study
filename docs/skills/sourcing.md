...
- **추가/갱신**: 스킬 CLI로 설치하면 lock에 source·`computedHash`가 기록된다(수동 편집 금지).
  ...
  ⚠️ 이전 문서는 이를 "절차 실행 불가"로 적었으나 **틀렸다** — 실제로 설치가 돌았다.
...
- **lock은 두 벌이고 성격이 다르다.**
  ...
  **전역 lock은 0종이 됐다** — 출처 규명의 정본이던 파일이 비었으므로,
  앞으로 출처 추적은 **프로젝트 lock의 `source`·`skillPath`** 로 한다.

- **`remove --all -g`는 전역 스코프에 갇히지 않는다** (실측)
...
> 실제로 그 경로가 발생했다 — `brainstorming`(`obra/superpowers`, 개인 계정, 실행 파일 4종)이
> lock에 등재됐다. **lock은 "안 바뀜"을 보장하지 "안전함"을 보장하지 않는다.**
> 그래서 축을 갈라 **출처 등급(A~C) × 고정 상태(🔒/⚙️)** 2차원으로 읽는다.

**등급은 출처가 아니라 「스킬」 단위로 매긴다** (재재개정 — 병렬 세션 지적으로 발견).
...
- 🔴 **실행 파일을 포함한 스킬은 등급과 무관하게 `security` 검토 대상**이다 —
  ...
  | `brainstorming` | **C** | `server.cjs`·`helper.js`·`start-server.sh`·`stop-server.sh` | ✅ 검토 완료(2,030행 전수) → 「거부」 → **「분리안」**: 이 4종은 **실행 범위 밖** |
...
🔴 **「위험 패턴 0건」은 「이 스킬은 전 범주 안전」이 아니었다** (재스캔 · 원칙 7 실사례).
첫 스윕의 **패턴셋은 `base64 -d`·`curl|bash`·평문 시크릿·`:latest` 4종**이었고,
**Spark writer 계열은 애초에 검색 대상이 아니었다.** 같은 패턴셋을 `spark-optimization`에
적용했을 때 K-1(High)로 걸린 바로 그 범주를, 이 스킬에서는 **아무도 보지 않았다**.

재스캔 실측(`grep -F`): `.mode("overwrite")` **3** · `saveAsTable` **4** ·
...
## C등급 5종 판정 (`security` — 6파일 2,539행 정독 + 패턴 스윕 6종)
...
**G-1은 스킬 하나가 아니라 통제 공백이다 — ✅ 결정됨(ⓑ 채택, 사용자 승인).**

> **결정**: 권고 ⓑ(**워커 지시문 명문화**)를 채택하고 ⓐ(`Edit(.github/workflows/**)` `deny`)는 **채택하지 않는다**.
> `deny`는 세션 전역이라 supervisor를 포함해 **모든 주체의 CI 편집을 닫는다** — 워크플로 작성이
> 정당한 업무인 이 저장소에서는 규칙을 빼야만 일할 수 있게 되고, 그것은 **게이트가 아니라 우회 습관**을 만든다.
> 대신 `.github/workflows/**`의 **소유자를 `devops-engineer` 단독으로 확정**하고(아래), 그 워커 지시문에
...
>   공개 위키 push를 발화**시키는데 프롬프트가 없었다. 이것은 G-1의 *시차* 공백과 **다른 축**이다 —
>   G-1은 "파일을 쓰고 나중에 CI가 실행"이고 이쪽은 "**에이전트가 지금 직접 발화**"라 `Bash` 매처가 볼 수 있다.
>
...
> ⚠️ **그중 둘은 이미 참이다** — `wiki.yml`이 위키 push를, `release.yml`이 릴리스 생성을 한다.
> 나머지 셋(레지스트리 push·`kubectl apply`·`terraform apply`)은 워크플로에 아직 없다.
> **트리거에는 조건과 함께 「현재 몇 개가 참인가」를 병기한다** — 그래야 다음 사람이 남은 것만 보면 된다.
> 이 절이 앞에서 적은 교훈(*"조건이 이미 참인지 먼저 센다"*)을 **바로 그 문단에서 절반만 적용**했다가
> 되짚은 자리다: 지난번은 *조건이 이미 지나갔고*, 이번은 *조건의 정의가 두 문서에서 갈렸다*.
> 형태는 다르지만 결과가 같다 — **트리거가 태어날 때부터 발동하지 않는다.**
> ⚠️ 지난 트리거(*"secrets가 처음 들어가는 시점"*)는 **설정 당시 이미 지나가 있었다** —
> `wiki.yml`이 그 전부터 `secrets.GITHUB_TOKEN`을 쓰고 있었다. **트리거를 걸 때는 그 조건이
> 이미 참인지 먼저 센다** — 미래형으로 적은 조건이 과거형이면 그 게이트는 태어날 때부터 죽어 있다.
...
나중에 추가한 `Bash(*kubectl apply*)` `ask`는 **여기에 닿지 않는다**.
...
`docker scout`의 실제 외부 전송 · 스킬 무결성(해시 재현 불가라 **"판정 시점 디스크 내용"에 대한 판정**이다).
