...
## 현행 (2026-08-27 **10:33 KST** — `brainstorming` 재설치·분리안 채택 후)

| 항목 | 값(10:33) | 2026-08-21 판(10:35) | 함의 |
| --- | --- | --- | --- |
| `~/.claude/skills/`(전역) | **0** | 0 | 전역 비움 유지 — 이름 충돌 축은 계속 닫혀 있다 |
| `.claude/skills/`(**프로젝트**) | **17** | 14 | +3 |
| `skills-lock.json` 등재 | **17** | 14 | +3 |
| **lock ↔ 디스크 일치** | ✅ **17 = 17** | ✅ 14 = 14 | 양방향 차집합 **0** (디스크에만·lock에만 둘 다 없음) |

- **`brainstorming`이 lock·디스크 양쪽에 있다** — 2026-08-21 제거분이 사용자에 의해 재설치됐고,
  2026-08-27 「분리안」이 채택됐다([`../skills.md`](../skills.md) §brainstorming).
  ⚠️ **등재는 실행 범위를 넓히지 않는다** — `scripts/**` 는 범위 밖이고 lock이 덮지도 않는다(B-2).
- ⚠️ **`17`은 디렉터리 수이지 검토 완료 수가 아니다.** `security` 검토를 통과한 것은 그보다 적고,
  그 분모는 [`../skills.md`](../skills.md) §① 표에서 센다. **두 수를 같은 문장에 놓지 마라.**
- **+3의 내역**: `find-skills`(`3cd65b9`) · `documentation`(`e936729`) — 둘은 커밋된 등재분이고,
  `brainstorming`은 **미커밋**이다(사용자 재설치분). 즉 **커밋된 lock은 16, 작업 트리는 17**이다.
  같은 파일의 두 상태를 같은 수로 세지 마라.
