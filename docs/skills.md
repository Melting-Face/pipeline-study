...
**supervisor 세션에서만** 쓸 수 있다. `skill-matcher`가 `dataviz`를 "죽은 참조"로 올린 것이
...
| `skill-matcher` | **없음** — 후보 탐색은 **`researcher` 릴레이** | 조사 요청서를 반환하고 회신 후보를 **채점·제안**한다(배선은 안 한다). 신뢰성 최종 판정은 `security` — [근거](skills/scoring.md#skill-matcher) |
