...
| **brainstorming** | `obra/superpowers` (**C**) | 🔁 **「분리안」 채택**(2026-08-27) — 마크다운 절차만 참조, `scripts/**` 실행 금지. **단서 필수** |
...
> ⚠️ **정정(2026-08-27 해시 재계산) — 위 B-2의 무결성 주장은 반증됐다.**
...
### 🔁 `brainstorming` — 「분리안」 채택 (2026-08-27) · 원판정 「거부」 (2026-08-21)
...
2026-08-27 채택했다. **예외 신설이 아니라 조항의 적용 범위 해석**이다 — `scripts/**` 를 범위에서 빼면
...
### `brainstorming` 단서 ✅ **유효**(분리안 채택 — 2026-08-21 본문 실측)
...
| `researcher` | **없음** | 등재 가능 **0건** — 이 워커는 CLI를 조작하지 않아 축1이 구조적으로 탈락한다 — [근거](skills/scoring.md#researcher) |
...
| `skill-matcher` | **없음** — 후보 탐색은 **`researcher` 릴레이**(2026-08-20) | 조사 요청서를 반환하고 회신 후보를 **채점·제안**한다(배선은 안 한다). 신뢰성 최종 판정은 `security` — [근거](skills/scoring.md#skill-matcher) |
...
| 2 | ~~`director`~~ × `brainstorming` | 🔁 **분리안 채택**(2026-08-27) — 원판정 ❌ 거부 | **워커 등재는 여전히 없다**(아래 ⓑ) |
| 3 | `data-engineer`·`analyst` × `sql-optimization` | — | 등재 자체는 유효(두 벌 **내용 동일**). ★ 재채점은 `skill-matcher` 소관 |

ⓑ **`brainstorming` 워커 등재가 없는 이유** — 이 행의 대상 워커 `director`는 **2026-08-23 폐기**됐다.
