...
> `-a`를 붙이자 여러 건이 나왔다. **그 0건을 그대로 읽었다면 "이력에 없음 → 에이전트 소행"으로 정반대 결론**이 났다
> ([philosophy.md](philosophy.md) 원칙 7의 교과서적 실사례 — 부정 결과는 관측 경로 생존을 함께 확인해야 유효하다).

**「디스크 수」가 무엇을 세는지 도구마다 갈린다.** 아래 값은 **내 환경의 그때 상태**이고
남의 환경에서 참이 아니다 — 세려면 직접 돌린다. 여기 남기는 이유는 값이 아니라
**같은 대상에 세 명령이 두 답을 낸다**는 것이고, 그것은 어느 환경에서나 참이다.
...
| B-9 | Medium | `--project-dir` 세션 산출물을 **의도적으로 안 지운다**(`/tmp`만 삭제). 정리 트리거 주체 부재 — "검증용 컴퓨트가 반나절 넘게 샜다"와 같은 형태 |
