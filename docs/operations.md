...
서브에이전트 로그는 그 아래 `<session-id>/subagents/`에 붙는다.
**쌓인 양은 세션 수의 함수라 자기 환경에서 잰다** — 실측은 볼트 §8-1.
...
| 항목 | 값·동작 |
| --- | --- |
| 설정 키 | `~/.claude/settings.json`의 `cleanupPeriodDays`(도구 기본 **30**) — ⚠️ **사용자 전역 설정이라 저장소가 정하지 않는다.** 이 환경의 값은 볼트 §8 |
...
```bash
# ✅ 세션 단위(정책과 같은 단위). 보존일 초과 세션이 0이면 정상
find ~/.claude/projects -maxdepth 2 -name '*.jsonl' -mtime +<보존일> | wc -l

# ❌ 재귀 탐색은 *파일*을 센다 — subagents/ 하위가 부모 수명으로 살아남아 0이 안 된다
find ~/.claude/projects -name '*.jsonl' -mtime +<보존일> | wc -l
```

**설정을 넣은 것과 정리가 도는 것은 다른 축이다.** 값을 바꿨으면 **대화형 세션을 한 번 띄운 뒤**
위 명령으로 확인하고, **대조군으로 `memory/` 무손상을 함께 센다**(삭제가 돌았다는 것과
지우면 안 될 것을 안 지웠다는 것은 다른 축이다).

⚠️ **잔여가 0이 아니어도 미삭제로 단정하지 마라.** 실집행에서 잔여 1건이 나왔는데
**미삭제가 아니라 위 재귀 명령이 세션이 아니라 파일을 세고 있었기 때문**이었다 —
`subagents/` 하위가 부모 수명으로 살아남는다. **값은 맞고 라벨이 틀린 경우**이고,
그래서 판정 전에 **어느 명령이 무엇을 세는지**부터 맞춘다([philosophy.md](philosophy.md) §계측 단위).
실집행 수치는 볼트 §8-1.
...
⚠️ **머신 자원 변경을 재생성 사유로 오해하지 마라.** 실측에서 VM 메모리를 상향한 뒤에도
클러스터 `lakehouse`와 PVC 2종이 **Bound 상태로 그대로** 살아 있었다(전후 값은 볼트 §6-4).
...
| 대상 | 담긴 것 | 소멸 시 |
| --- | --- | --- |
| `catalog-postgres-1` | Iceberg JDBC 테이블 메타 | ❌ 테이블 정의 소실 — 재적재해야 복구 |
| `data-seaweedfs-0` | 원천 csv.gz + Iceberg parquet | ⚠️ parquet 재생성은 전 파이프라인 재실행 |
| 로컬 레지스트리 | 러너 이미지 | ✅ 재빌드·재push |
| Flink 체크포인트 | 스트리밍 상태 | ✅ PoC라 무의미 |

📌 **크기는 여기 적지 않는다** — 적재량의 함수라 남의 환경에서 참이 아니다. 실측은 볼트 §8-2.
**비용을 가르는 것은 크기가 아니라 위 「소멸 시」 열**이다(재적재 가능성).

⚠️ **`df`·`du`·버킷 합계가 각각 다른 것을 센다.** `df -h /data`는 **노드 디스크 전체**
(containerd 이미지 레이어 포함), `du -sh /data`는 **preallocate된 sparse 볼륨 파일의 예약분**이지
데이터량이 아니며(같은 파일이 `ls -la`로는 수십 KB), 버킷 합계가 **실제 오브젝트**다.
⚠️ **`du`를 백업 비용으로 읽지 마라** — 실측에서 둘이 약 100배 벌어졌다.
용량 계획엔 `du`, **백업 비용엔 버킷 합계**를 쓴다 — 세 값의 대조는
[resource-sizing.md](resource-sizing.md) §disk와 볼트 §6-3.
