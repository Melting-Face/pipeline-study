...
> 선택지는 (가) 경계 ① 개정 / (나) **시연 창 안에서만 돌리고 그 자리에서 회수**였고,
> ✅ **(나)로 확정됐다(2026-08-23 사용자 결정)** — **경계 ①은 개정하지 않고 적용 범위를 명시**한다.
> **2026-08-23 실증도 (나)로 진행**했고 회수 후 기준선(`2250m / 3140Mi`)으로 정확히 복귀했다.
> 🔴 **상주 피크가 처음 측정됐다 — `4750m (59%) / 8772Mi (39%)`**(JM + TM + Spark Connect,
> 분모는 노드 Allocatable). **배치 동시 피크(84% / 52%)보다 낮다** — 즉 **예산은 남는다.**
> 그럼에도 (나)인 이유는 **회수 규율**이며(2026-08-19 13시간 유출 전례),
> **"예산이 통과한다"와 "경계 ①의 전제가 성립한다"는 다른 축**이다.
> **스트리밍 상시 운전은 하지 않는다** — 상시로 올릴 때가 곧 재검토 트리거다.
> 정본 [conventions/k8s.md](conventions/k8s.md) §9-3 §경계 ①의 스트리밍 단서 ·
...
- **Check**: 스트림 입력 대비 경보 산출 정확성·지연 관측, 배치(Spark)와 **동시 기동** 시 경계 3개 준수 확인
  ...
  상주분은 **2026-08-23에 측정**됐고 같은 날 **(나) 시연 창 한정 + 즉시 회수로 확정**됐다
  (경계 ① 무개정 · 잡 제출 **전에** 회수 시점을 정한다). 경계 ③은 Redpanda 미도입으로 **발동하지 않는다.**
...
| 비용 | ★★★☆☆ | 로컬이라 클라우드 비용 0, 단 **단일 PC RAM 압박**(2엔진+SeaweedFS). **8 CPU / 22,888 MiB** VM에서 **동시 기동**으로 수용(실측 피크 CPU 84% / Mem 52%). 🔴 **Redpanda는 미도입 유지로 결정**돼(2026-08-23) 이 축의 증분이 없어졌다. **스트리밍 TM 상주분은 같은 날 실측됐다 — 상주 피크 `4750m (59%) / 8772Mi (39%)` 로 배치 동시 피크보다 낮다.** 🔴 그럼에도 §9-3 경계 ①은 **개정하지 않고 (나) 시연 창 한정 + 즉시 회수로 확정**됐다(2026-08-23 사용자 결정) — **예산이 아니라 회수 규율**이 사유다([resource-sizing.md](resource-sizing.md) · [conventions/k8s.md](conventions/k8s.md) §9-3) |
