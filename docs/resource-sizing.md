...
⁶ 🔴 **"일시"는 배치 잡 기준이다** — **스트리밍 잡의 TM은 잡 수명 내내 상주**한다(2026-08-23 실측).
  ...
  §9-3 **경계 ①의 전제**(온디맨드·수명 1분 미만)는 여전히 깨진다 — 그래서 **(나) 시연 창 한정 +
  즉시 회수로 확정**됐고(2026-08-23 사용자 결정) **경계 ①은 개정하지 않는다**
  (§(C) "스트리밍 시" 블록 · §(C-2) 하위 §스트리밍 상주 피크).
  🔴 **따라서 이 표는 그대로 둔다** — TM 상주는 **상시 구성이 아니라 시연 창 안의 일시 구성**이기 때문이다.
  상시로 돌려야 할 때 이 표를 **TM 상주 기준으로 재실측**한다.
...
> 🔴 **스트리밍 시 — 경계 ①의 전제가 깨진다 (✅ 측정 완료 · ✅ (나)로 확정 · 2026-08-23)**
>
...
>
> | 선택지 | 내용 | 이 문서에 미치는 영향 | 판정 |
> | --- | --- | --- | --- |
> | (가) | 경계 ① 자체를 개정한다(TM 상주를 예산에 반영) | §(B)·§(C-2)를 **TM 상주 기준으로 재실측** | ❌ 미채택 |
> | (나) | **시연 창 안에서만 돌리고 그 자리에서 회수** | 현행 표 유지(회수 규율에 그대로 얹힘) | ✅ **채택** |
>
> ✅ **(나)로 확정됐다(2026-08-23 사용자 결정)** — 2026-08-23 스트리밍 실증도 **(나)로 진행**했고,
> **경계 ①은 개정하지 않는다.** ⇒ **이 문서의 표는 그대로 유지**된다.
> ✅ **`미측정`이던 칸은 채워졌다** — 스트리밍 상주 피크 **`4750m (59%)` / `8772Mi (39%)`**
> (§(C-2) 하위 §스트리밍 상주 피크). **배치 동시 피크(84% / 52%)보다 낮다.**
> 🔴 **그 수치는 "허용 근거"가 아니다** — **예산은 남지만 회수 규율 때문에 상주시키지 않는다.**
> **예산 통과와 전제 성립은 다른 축**이고, (가)를 택하면 잃는 것이 하나 더 있다:
> *"TM이 떠 있으면 잡이 도는 중"* 이라는 **`kubectl get pods` 한 번으로 되는 이분법**이다
> (회색 지대의 판정에는 누적 시간 계측이 필요한데 그 계측기가 없다).
> **재검토 트리거**는 스트리밍을 시연이 아니라 **상시**로 돌려야 할 때다 — 그때 §(B)를 TM 상주 기준으로
> 다시 쓰고, 재측정 시 **긴 쪽을 먼저 띄우는 순서 규칙**(아래 §(C-2) 주의)을 그대로 적용한다.
> 결정 전문·운영 규칙은 정본 [conventions/k8s.md](conventions/k8s.md) §9-3 §경계 ①의 스트리밍 단서.
...
> 🔴 **경계 ①과의 충돌 — 측정됐고 ✅ (나)로 확정됐다(2026-08-23 사용자 결정).**
...
> 규약을 자동으로 맞춰 주지 않는다.
> ⇒ **경계 ①은 개정하지 않고**, 스트리밍 잡은 **시연 창 한정 + 즉시 회수**로 돌린다.
> 🔴 **위 표의 스트리밍 상주 피크는 "상주해도 된다"의 근거가 아니라 "예산은 남았으나 회수 규율로
> 상주하지 않는다"의 근거**다. **이 문서는 규약을 바꾸지 않으며**, 결정 전문·운영 규칙
> (제출 전 회수 시점 확정 · `flink cancel` → `FlinkDeployment` 삭제 순 · 회수 후 기준선 복귀 확인)은
> 정본 §9-3 **§경계 ①의 스트리밍 단서**에 있다.
