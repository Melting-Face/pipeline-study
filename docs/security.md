...
| `usgs_water` | **없음** — 하천 관측소 계측값이고 개인 단위 관측이 아니다 | **미확인** — 조사하지 않았다 | 통제 **유지**(원천 값을 저장소·공개물에 담지 않는다). 재검토 트리거는 아래 |
...
- 두 데이터셋 공통 — **원천 값·그 파생 수치를 저장소나 공개물(리포트·위키·발표자료)에 담으려 할 때.**
  그때 `frankfurter_fx`는 출처를 표기하고, `usgs_water`는 **먼저 재배포 조건을 조사**한다.
