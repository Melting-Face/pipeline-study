...
이 프로젝트가 지키는 보안·데이터 취급 **정책**을 **ISMS-P 인증기준**과 **의료데이터 보안 규제**에
매핑해 한 곳에서 밝힌다.
...
> 적는 자리가 아니다 — 그것은 저장소 밖 비공개 기록의 몫이다.
>
> 🔴 **어법이 그 경계를 가른다.** 같은 사실이라도 **"범위 밖"**(이 통제로 보증하지 않기로 정했다 —
> 시제 없음)과 **"미적용"**(지금 아직 안 했다 — 낡는다)은 다른 축이고, 여기 쓰는 것은 **앞의 것**이다.
> 2026-08-24 세 칸이 뒤엣말로 새어 있었다.
