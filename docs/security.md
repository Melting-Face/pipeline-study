...
> **목적**: 이 프로젝트가 지키는 보안·데이터 취급 **정책**을 **ISMS-P 인증기준**과
> **의료데이터 보안 규제**에 매핑해 한 곳에서 밝힌다.
> **언제 읽나**: 보안 통제 추가·점검, 데이터 취급 정책 결정, 실서비스 확장 검토 시.

> 🔴 **이 문서가 담지 않는 것 — 현행 실태·미비점·미해소 항목.**
> 통제가 *지금 어디까지 적용돼 있는지*, 무엇이 미해소인지는 **이 문서에 없다.**
> 저장소 밖 비공개 기록(`$OBSIDIAN_VAULT/security/posture.md`)에서 관리한다(2026-08-23 분리).
> **없는 것과 안 적은 것을 구분하기 위해 이 사실을 선언한다** — 아래 각 통제의
> **「보증 범위」** 열이 *어디까지를 보증하는지*는 밝히되, 미적용 지점의 구체는 밝히지 않는다.
...
- 이 저장소에는 **원천 진료 데이터·크리덴셜이 포함되지 않는다**(§0 철칙). 그런 파일을 발견했다면
...
> 🔴 **「보증 범위」 열은 원칙 7의 적용이다.** 통제가 있다는 사실은 그 통제가 *무엇까지* 보는지와
> 다른 축이며, 범위를 안 적으면 **선언이 실효로 읽힌다.** 반대로 이 열은 *현재 어디가 뚫려 있는지*를
> 적는 자리가 아니다 — 그것은 비공개 posture 기록의 몫이다.
...
| **2.7 암호화 적용** | 비밀정보는 하드코딩하지 않고 참조로 주입한다(`dg.EnvVar`·`${ENV:...}`). 개인키·`*.tfstate`·`*.tfvars`·kubeconfig는 gitignore + 권한 `600` | 전송 구간과 비밀의 저장소 유입까지. **저장 암호화(at-rest)는 미적용**이며 실서비스 확장의 전제다([§4-2](#4-2-저장전송-암호화-27)) |
...
| **2.10 시스템 및 서비스 보안관리** | UTC 저장 / KST 표시로 로그 타임스탬프를 정합화한다 | 타임스탬프 일관성까지. 중앙 감사 로그 수집·보관은 미설정이다([§4-5](#4-5-감사-로그접속기록-210--32)) |
| **2.11 사고 예방 및 대응** | 관측 수단은 **무엇을 두고 무엇을 안 두는지 선언**한다("안 둔다"도 선언한다 — 빠뜨린 것과 구분하기 위해). 규칙 정본은 `conventions/monitoring.md` | **규칙 문서까지.** 침해 대응 절차·알림 경로는 미정의다. ⚠️ **문서가 생긴 것이 통제가 생긴 것은 아니다** |
...
3. 🔴 **데이터 반출** — `data-extractor`의 추출물. 착지는 **저장소 밖** `$DATA_EXTRACT_DIR`
   (기본 `~/extracts`)이며, **앞의 둘과 달리 원천 진료 데이터 그 자체**가 나간다.
...
| 개별 행 노출 차단 | 리포트에 개별 환자 행 금지, **소규모 셀(관례상 5 미만) 마스킹** | 3.3 · DUA |
...
두 건의 실측에서 얻은 규칙이며, **둘 다 "관측 경로는 살아 있었다"** 는 공통점을 갖는다.
⇒ 🔴 **「관측 경로의 생존」과 「관측 범위의 충분성」은 다른 축이다.**

1. 🔴 **크리덴셜 유출 점검 범위에는 숨김 파일(`.*`)·셸 히스토리·홈 디렉터리를 반드시 포함한다.**
   로그 디렉터리만 훑는 점검은 **범위 밖을 조용히 부정 결과로 계상**한다.
2. 🔴 **방어 도구가 여럿이면 각자 무엇을 보는지 표로 적는다.** *"몇 겹인가"* 는 안전의 척도가
   아니다 — **겹의 합집합이 대상을 덮는가**만이 척도다. 도구마다 보는 대상이 달라 각자
   자기 범위에서 정상 작동해도 **틈이 남을 수 있고, 전부 초록불이라 합집합이 전부를 덮는다고 읽힌다.**

> 두 규칙은 처방이 갈린다 — 1은 **한 도구의 범위 지정이 빗나간** 경우, 2는
> **여러 도구의 범위가 안 겹쳐 틈이 생긴** 경우다. 근거가 된 실측 2건은 posture 기록에 있다.
