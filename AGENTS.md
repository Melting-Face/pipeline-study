...
품질 **차원**과 실패 시 등급은 테스트 계층과 다른 축이다. 위 목록은 *어떻게 테스트하나*이고,
*무엇을 품질로 보고 깨지면 무엇을 하나*는 `docs/conventions/data-quality.md`가 정본이다.
테스트를 추가할 때는 실패 등급을 차단·경고·관측 중 하나로 명시한다.

리스크 등급은 영향 × 가능성에 **가역성**을 더해 매기고 비가역이면 한 단 올린다.
비가역 작업 목록과 대응 전략은 `docs/risk.md`가 정본이며, 개별 리스크 항목은
저장소가 아니라 Issue 또는 볼트에 남긴다.

사고가 실제로 났을 때의 순서도 같은 문서 §7이다. 원인보다 도달 범위를 먼저 묻고,
확산 정지 → 증거 보존 → 수정 순으로 간다. 재기동·재생성은 증거를 지우므로 상태를
남기기 전에 하지 않으며, 복구 조치가 비가역이면 평시와 같은 승인 게이트를 탄다.
