...
- **에이전트 오케스트레이션·기록관**: AI 세션을 **2계층(supervisor → worker)** 으로 나눈다.
  ...
  **worktree는 의무다** — 루트 쓰기·커밋은 `worktree_guard.py`가 `deny`한다(예외 2종 고정,
  대상이 **저장소 밖이면 통과**). 생성은 **`scripts/worktree-new.sh`**(맨손 `add`는 **피어 감지를
  조용히 끈다**), 이주는 `EnterWorktree`. **배선은 새 세션부터** 듣고 그전엔 **pathspec 의무**가
  유일한 방어선이다. ⚠️ **문자열 휴리스틱이라 봉쇄가 아니다**(한계는 `git/worktree.md`).
