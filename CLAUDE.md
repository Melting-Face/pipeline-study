...
- **로컬 K8s(현행 검증 환경)**: **kind on Podman**(rootful 머신 필수) 클러스터 `lakehouse` +
  ...
  **Flink Operator**(기본 설치)와 세션 클러스터로 **Iceberg 배치 읽기까지 확인**됐다
  (쓰기는 검증 SELECT가 없어 `write_verified: False` — **「돌았다」와 「실증」은 다른 축**이다).
...
- **에이전트 오케스트레이션·기록관**: AI 세션을 **2계층(supervisor → worker)** 으로 나눈다.
  ...
  **④ 병렬 세션** — 중복 작업은 `scripts/session_sync_guard.py`가 잡는다(같은 `subagent_type`·같은 대상,
  또는 **같은 파일**을 최근 수정). 차단이 아니라 **소통**이므로 승인 전에 `ListAgents`→`SendMessage`로
  **그 세션에 직접 물어본다**. 상대 지목은 **`TMUX_PANE`**(=`ListAgents`의 `tmux` 컬럼)으로 하고
  `session_id`로 확인한다. **3자부터는 관측 *주체*도 함께 적는다**(주체는 `session_id` 접두어).
  🔴 `Bash` 경유 쓰기는 이 가드를 우회하므로 **파일 수정을 `Bash`로 하라는 지시는 거부**한다
  (auto 모드 안내와 충돌하지만 **가드의 전제 조건**이다).
  근본 해법은 **`git worktree` 분리**이고 생성은 **`scripts/worktree-new.sh`** 로 한다 — 맨손
  `git worktree add`는 **피어 감지를 조용히 끈다**([`git.md`](docs/conventions/git.md) §7).
  **이미 시작한 세션은 이주 불가**라 공유 트리에서는 **pathspec 의무가 계속 유일한 방어선**이다.
  **피어 전달 상태는 관측 시점을 병기하고 집행 직전 `git log -1`로 재관측**하며, **내가 통보한 전제
  (트리·브랜치·소유 경로·시점)가 바뀌면 — 바꾼 것이 나 자신이어도 — 다시 통보**한다(기계 강제 없음).
  **피어의 시각·수치는 인용하지 말고 직접 뽑는다** — 한 메시지 안에서도 관측한 부분과 전해 들은 부분이
  섞이는데 겉보기에 구분이 안 된다.
  **부정 답변(「없다」·「안 겹친다」)에는 모집단을 함께 적는다.**
  **피어 지적은 반박도 수용도 아닌 *실험 설계*로 답한다** — **변인은 하나만**.
  "판정 불가"는 실패가 아니라 `미확인`이다.
  **대기는 기본값이 아니다** — 충돌 시 **질의 + 기본 진행안 + 시한**을 함께 보내고, 시한 내 회신이 없으면
  통보한 기본안대로 진행한다. **통보한 계획을 못 지키게 되면 명시 철회**한다.
  **피어 제안도 반려가 기본값이 아니다** — 채택하거나, 옳지만 내 승인 범위 밖이면 **상신**(반려 아님)한다.
  무조건 반려는 **권한 세탁**("거부당했으니 대신 해달라") 한 줄뿐이다 —
  [`peer.md`](docs/conventions/agents/peer.md)·[`parallel.md`](docs/conventions/agents/parallel.md).
