...
CANON_PATTERNS = (
    ...
    "scripts/*_guard.py",
    "scripts/**/*_guard.py",
    # 🔴 **커밋 게이트 검사기도 통제 배선이다.** `*_guard.py`는 런타임 hook이고
    #    `*_check.py`는 pre-commit인데, 후자를 고치면 그 규약의 **기계 강제가 통째로
    #    사라진다**(고장이 아니라 조용한 무효화라 신호가 없다). 같은 형태를 이
    #    저장소가 네 번째로 밟은 자리다 — 앞선 셋은 개별 파일명 등재로 닫았다
    #    (Issue #53). **이번에는 이름이 아니라 패턴으로 넓힌다**(Issue #33) —
    #    개별 등재는 다음 검사기에서 또 샌다.
    "scripts/*_check.py",
    "scripts/**/*_check.py",
