...
| 상황 | 스킬 | 하지 말 것 |
| --- | --- | --- |
| Dockerfile 태그 고정·스테이지 분리 감사 기준 | `multi-stage-dockerfile` | **감사 기준은 정본 우선**(아래 충돌 규칙) — 스킬 체크리스트를 기준으로 삼지 않는다 |
| manifest·RBAC·리소스 설정 감사 기준 | `kubernetes-specialist` | 🔴 아래 §실행 금지 패턴이 **호출의 조건**. 보안 항목은 `security` 소관으로 넘긴다 |
| Terraform 버전 고정·lock 커밋·파일명 관례 감사 | `terraform-style-guide` | 감사 항목은 버전 고정·`.terraform.lock.hcl` 커밋·역할별 파일명 |
...
- 🔴 **표 밖 스킬을 호출하지 마라.** 감사자에게 **초과 스펙**인 것들이다 —
  `spark-optimization`(너는 "한도가 **선언돼 있는가**"만 보면 되고 기준은
  [`resource-sizing.md`](../../docs/resource-sizing.md)가 정본이다) ·
  `terraform-test`·`terraform-stacks`(관행·제품을 **채택한 적이 없어** 감사 기준 자체가 없다).
  CI 게이트 설계·`scripts/*.sh` 검사는 [`terraform.md`](../../docs/conventions/terraform.md)·
  [`docker.md`](../../docs/conventions/docker.md)·[`test.md`](../../docs/test.md) 조항을 직접 대조한다(스킬이 없다).
