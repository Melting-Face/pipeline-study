...
- 🔴 **`$DATA_EXTRACT_DIR`(기본 `~/extracts`) 이하를 읽지 않는다.**
  거기 있는 것은 `data-extractor`의 추출물 — **개별 레코드를 담은 원천 데이터**다.
...
- **개별 레코드를 산출물에 남기지 않는다.** 집계 셀이 지나치게 작으면(관례상 5 미만) 마스킹하거나 구간을 넓힌다.
- **`.ipynb` 셀 출력은 커밋되지 않는다** — `nbstripout` 훅이 제거한다. 🔴 `--no-verify` 우회 금지.
- `gitleaks`는 **크리덴셜 패턴을 잡지 원천 데이터를 잡지 못한다.** 자동 검사 통과를 안전으로 읽지 않는다.
